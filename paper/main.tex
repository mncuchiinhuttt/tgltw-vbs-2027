...
\begin{tikzpicture}[
  ...
  % Lane 3: KIS-C
  \node[lane, below=11mm of l2] (l3) {KIS-C};
  \node[pbox, right=3mm of l3] (c1) {turn + history};
  \node[pstage, right=3mm of c1] (c2) {entity CQR};
  \node[popt, right=3mm of c2] (c3) {ambiguity test\\[-1pt]\tiny $A < 0.7$};
  \node[pstage, right=9mm of c3] (c4) {n-gram boost};
  \node[pstage, right=3mm of c4] (c5) {negative filter};
  \draw[arrow] (c1) -- (c2); \draw[arrow] (c2) -- (c3);
  \draw[arrow] (c3) -- (c4);
  \draw[arrow] (c4) -- (c5);
  \draw[arrow, dashed] (c5.north) -- ++(0, 4.6mm) coordinate (c5top)
    -- (c5top -| c1.north) coordinate (c1top)
    -- (c1.north);
  \node[font=\tiny, align=right, anchor=east] at ([xshift=-1mm]c1top) {operator\\feedback};
