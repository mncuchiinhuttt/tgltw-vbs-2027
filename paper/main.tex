...
\begin{abstract}
Timed interactive video retrieval requires fast candidate exploration under strict penalties for false submissions. We present \textbf{AEGIS} (\textbf{A}daptive \textbf{E}vidence-\textbf{G}rounded \textbf{I}nteractive \textbf{S}earch), a new evidence-grounded interactive video retrieval system for the Video Browser Showdown (VBS 2027). AEGIS combines Tencent WeMM-Embedding-4B representations with Qdrant HNSW indexing and weighted Reciprocal Rank Fusion, while its KIS-C engine preserves entities across turns, detects ambiguity, boosts phrase matches, and filters rejected visual attributes. Parallel fail-closed VQA grounds answers in localized media, and an intra-video timeline explorer supports evidence verification; on V3C, AEGIS achieves 57.9\% Recall@1, 74.7\% Recall@5, and a Mean Reciprocal Rank (MRR) of 0.665 on a 200-query stress test, with 96.7\% grounded VQA exact match and 0.0\% ungrounded hallucination.
...
Recent competitive systems~\cite{jackl2026prak,tran2026niiuit,khan2026exquisitor} show that live retrieval requires sub-second candidate exploration and high-precision discrimination between visually similar scenes. Examples include repetitive shots from the same video, visually similar wedding or marine scenes, and surgical frames with small instrument changes. Existing architectures face three operational bottlenecks:
...
To address these challenges, team \textbf{TGLTW-RMIT} introduces \textbf{AEGIS}. Our primary technical contributions are:
\begin{enumerate}\setlength{\itemsep}{1.5pt}\setlength{\parskip}{0pt}
    \item \textbf{High-Capacity Representation with MRL}: We deploy Tencent WeMM-Embedding-4B~\cite{wemm2025} with Matryoshka Representation Learning (2,048d vectors) indexed via Qdrant HNSW~\cite{malkov2018efficient,qdrant} ($M=16, ef=128$). We choose this model for its unified image-text representation and fine-grained scene coverage; SigLIP adds complementary visual similarity.
    \item \textbf{Peak Conversational Search Engine (KIS-C)}: We design an entity-preserving Contextual Query Rewriting (CQR) pipeline with Dynamic Video Ratio (DVR), Score Margin Ambiguity (SMA), compound $n$-gram phrase clarification boosting~\cite{sekulic2021evaluating}, and conversational negative-feedback dampening.
    \item \textbf{Parallelized Fail-Closed Grounded VQA}: We engineer an $8\times$ concurrent ThreadPool pipeline that resolves physical media provenance, applies localized YOLOE-26 crops, and enforces a strict fail-closed contract (\code{UNKNOWN/N/A} on ungrounded candidates). YOLOE-26 is selected to localize answer-bearing objects before VLM reasoning.
...
\node[box, minimum width=2.4cm] (assets) {\textbf{Video assets}\\V3C, MVK, LHE/GynSurg,\\metadata, ASR, shots};
\node[stage, right=4.5mm of assets, minimum width=2.6cm] (offline) {\textbf{Offline processing}\\WeMM-4B + SigLIP,\\PP-OCRv6, Whisper};
\node[stage, right=4.5mm of offline, minimum width=2.6cm] (index) {\textbf{Qdrant HNSW index}\\2048d visual vectors,\\enriched text payload};
...
AEGIS introduces an evidence-carrying, live-first multimodal retrieval architecture for VBS 2027. By coupling 4B-parameter WeMM-Embedding representations with Qdrant HNSW indexing, multi-turn compound $n$-gram clarification boosting, fail-closed grounded VQA, and interactive timeline exploration, the system delivers sub-second responsiveness with verifiable accuracy. Future work includes live DRES rehearsal for the VBS 2027 session at MMM 2027 in Siem Reap, Cambodia, expanded evidence models for marine and surgical collections, tighter retrieval/VLM budget calibration, and compiler-assisted inference optimization.
\clearpage
