...
Tables~\ref{tab:ablation-retrieval-kisc} and~\ref{tab:ablation-vqa-scaling} are internal AEGIS ablations, not comparisons with prior systems. Each row adds one component under the same protocol, isolating the effects of fusion, KIS-C clarification, grounded VQA, concurrency, and precision escalation.
\begin{table}[t]
\centering
\caption{Internal AEGIS ablation studies: retrieval fusion and multi-turn KIS-C dynamics.}
...
\begin{table}[t]
\centering
\caption{Internal AEGIS ablation studies: grounded VQA, concurrency scaling, and precision ladder.}
...
\vspace{-3mm}
\end{table}
As documented in Tables~\ref{tab:ablation-retrieval-kisc} and~\ref{tab:ablation-vqa-scaling}, fusion improves Recall@1 and MRR; KIS-C reduces ambiguity; fail-closed VQA eliminates unsupported answers; parallelism reduces latency; and the precision ladder trades latency for recall.
\section{Conclusion}
