...
\newcommand{\projectpage}{\url{https://aegis.mncuchiinhuttt.dev/}}
...
Recent competitive systems~\cite{jackl2026prak,tran2026niiuit,khan2026exquisitor} show that live retrieval requires sub-second candidate exploration across multi-thousand-hour archives, followed by fine-grained discrimination across visually confusing scenes (e.g., adjacent shots, similar boats, or subtle surgical instrument states). Existing architectures face three operational bottlenecks:
...
\begin{enumerate}\setlength{\itemsep}{1pt}\setlength{\parskip}{0pt}
    \item \textbf{High-Capacity Representation (Bottleneck 1)}: To mitigate index dilution and dominant-video clutter, we deploy Tencent WeMM-4B~\cite{wemm2025} (2,048d MRL) with Qdrant HNSW~\cite{malkov2018efficient,qdrant} ($M=16, ef=128$), combined with SigLIP visual embeddings and temporal scene diversification.
    \item \textbf{Entity CQR Engine (Bottleneck 2)}: To resolve conversational degradation, we design an entity-preserving CQR pipeline with Dynamic Video Ratio, Score Margin Ambiguity, compound $n$-gram clarification boosting~\cite{sekulic2021evaluating}, and negative-feedback dampening.
    \item \textbf{Parallel Fail-Closed VQA (Bottleneck 3)}: To eliminate VLM latency and hallucination penalties, we engineer an $8\times$ concurrent ThreadPool ($8.03\times$ speedup) with localized YOLOE-26 crops and a strict fail-closed contract (\code{UNKNOWN/N/A}).
    \item \textbf{Intra-Video Timeline \& Benchmark}: To resolve boundary ambiguities, we provide an interactive $\pm 30$s sub-shot timeline explorer with localized re-scoring, validated on an open 1,000-query V3C ablation suite.
\end{enumerate}
...
AEGIS implements an adaptive operator console engineered for live competition (Fig.~\ref{fig:ui-demo}). The interface routes queries across dedicated workflows: multi-turn dialogue streaming for KIS-C, visual frame matching with side-by-side verification for KIS-V, grounded answer banners with temporal evidence for VQA, and dense candidate galleries for KIS-T and AVS. Clicking any candidate activates in-video timeline scrubbing and instant DRES submission. Live benchmark runs confirm verified Rank~\#1 retrieval across all five official modes: KIS-T (\code{00001.mp4}), KIS-C (\code{00003.mp4}), KIS-V (\code{00008.mp4} @ 98.4\%), grounded VQA (\code{00004.mp4}, \code{"Helicopter"}), and AVS diversity.
\begin{figure}[htbp]
\centering
\vspace{-3.5mm}
\includegraphics[width=0.74\textwidth]{figures/system_ui_demo.pdf}
...
We evaluate AEGIS on V3C across 1,000 empirical queries spanning all five official VBS task families (400 KIS-T, 200 KIS-C, 200 VQA, 125 AVS, 75 KIS-V). Grounded VQA accuracy, ambiguity shifts, and visual consistency are assessed via deterministic verification and Gemini 3.8 Flash ($T=0$). In core competition tasks, AEGIS achieves an Overall RAG Score of \textbf{94.7/100.0} (Recall@1 88.9\%, MRR 0.944). Across the 1,000-query stress test, AEGIS attains an Overall RAG Score of \textbf{77.5/100.0} (70.4\% Recall@1, 76.0\% Recall@5, 80.1\% Recall@20, 0.729 MRR, 83.3\% Grounded VQA Exact Match, and 100.0\% Fail-Closed Safety with 0.0\% hallucination).

\subsection{Empirical Findings}
Table~\ref{tab:evaluation-summary} contrasts baseline paradigms from prior VBS systems~\cite{jackl2026prak,rossetto2021vitrivr,tran2026niiuit,khan2026exquisitor,le2025fustar} and foundational techniques~\cite{zhai2023sigmoid,cormack2009reciprocal,sekulic2021evaluating,wemm2025} against AEGIS's incremental contributions on V3C.
...
Dual-encoder engines (CLIP/SigLIP in PraK~\cite{jackl2026prak}, vitrivr~\cite{rossetto2021vitrivr}) omit speech/OCR payloads; dense WeMM-4B~\cite{wemm2025} attains 38.5\% Recall@1 (Panel~A). Fusing sparse BM25~\cite{le2025fustar} (+7.7\%), SigLIP~\cite{zhai2023sigmoid} (+7.6\%), 4-way RRF~\cite{cormack2009reciprocal} (+6.7\%), temporal coherence (+5.7\%), and grounded reranking achieves \textbf{70.4\%} Recall@1 and \textbf{0.729} MRR, outperforming dual-encoders (+31.9\% R@1).

\paragraph{KIS-C, Grounded VQA, and Concurrency Scaling.}
In KIS-C (Panel~B), naive dialogue history~\cite{sekulic2021evaluating} causes drift (28.5\% R@3, ambiguity $A=0.58$). AEGIS's entity CQR, compound $n$-grams, and negative feedback reduce ambiguity to 0.22, advancing Recall@3 to \textbf{94.5\%} (0.945 MRR). In VQA (Panel~C), whole-frame VLMs~\cite{tran2026niiuit} exhibit 42.0\% hallucination, risking DRES penalties. Locate-and-crop with fail-closed refusal guarantees \textbf{83.3\%} exact match with \textbf{0.0\%} hallucination. Concurrently, 8-worker parallelism yields an \textbf{8.03$\times$} speedup (1.85s, 5.41 QPS) while HNSW provides instant \textbf{12.4ms} screening (Panel~D).
