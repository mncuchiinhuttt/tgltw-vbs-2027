...
\begin{enumerate}\setlength{\itemsep}{1.5pt}\setlength{\parskip}{0pt}
    ...
    \item \textbf{Intra-Video Timeline Explorer \& Decoupled RAG Suite}: We build an interactive sub-shot browser providing $\pm 30$s shot context with localized re-scoring, validated through a reproducible 1,000-query corpus ablation package.
\end{enumerate}
...
Tables~\ref{tab:ablation-retrieval-kisc} and~\ref{tab:ablation-vqa-scaling} isolate the empirical gains of fusion, KIS-C clarification, grounded VQA, concurrency, and precision escalation under identical protocols on V3C.
\begin{table}[t]
\centering
...
\midrule
\multicolumn{5}{l}{\emph{Panel A: Multimodal Retrieval Fusion (1,000-Query Corpus Test)}} \\
(M1) Dense Only (WeMM-4B) & 38.5\% & 52.1\% & 0.448 & 0.038s \\
(M2) Dense + Sparse BM25 Payload & 46.2\% & 58.4\% & 0.518 & 0.052s \\
(M3) Dense + BM25 + SigLIP Secondary & 53.8\% & 64.0\% & 0.589 & 0.076s \\
(M4) + 4-Way Weighted RRF Fusion & 60.5\% & 69.2\% & 0.645 & 0.084s \\
(M5) + Temporal Coherence \& Diversify & 66.2\% & 73.1\% & 0.694 & 0.092s \\
(M6) + Evidence-Grounded Fusion (Full Engine) & \textbf{70.4\%} & \textbf{76.0\%} & \textbf{0.729} & \textbf{0.477s} \\
\midrule
\multicolumn{5}{l}{\emph{Panel B: Conversational KIS-C Multi-Turn Progression (200 Scenarios)}} \\
(C1) Turn 1: Initial Vague Query & 0.0\% & 38.0\% (R@3) & 0.265 & 0.088s (Amb 0.61) \\
(C2) Turn 2: Naive History Concat & 28.5\% & 54.0\% (R@3) & 0.412 & 0.092s (Amb 0.58) \\
(C3) Turn 2: + Entity-Preserving CQR & 58.0\% & 74.0\% (R@3) & 0.655 & 0.110s (Amb 0.52) \\
(C4) Turn 2: + Compound N-gram Boost & 82.5\% & 89.0\% (R@3) & 0.860 & 0.115s (Amb 0.41) \\
(C5) Turn 3: + Negative Filter \& Feedback & \textbf{94.5\%} & \textbf{94.5\% (R@3)} & \textbf{0.945} & \textbf{0.120s (Amb 0.22)} \\
\bottomrule
\end{tabular}%
}
\vspace{-3mm}
...
\midrule
\multicolumn{5}{l}{\emph{Panel C: VQA Grounding \& Fail-Closed Safety (200 Queries)}} \\
Ungrounded Whole-Frame VLM & Exact Match: 52.0\% & 58.0\% & 42.0\% (Hazardous) & 1.42s \\
Locate-and-Crop (YOLOE-26 + VLM) & Exact Match: 76.5\% & 82.0\% & 12.0\% & 1.55s \\
AEGIS Fail-Closed Grounded Contract & \textbf{Exact Match: 83.3\%} & \textbf{92.5\%} & \textbf{0.0\% (Zero Error)} & \textbf{0.468s (100\% Safe)} \\
\midrule
...
\vspace{-3mm}
\end{table}
As documented in Tables~\ref{tab:ablation-retrieval-kisc} and~\ref{tab:ablation-vqa-scaling}, across 1,000 queries, fusion achieves 70.4\% Recall@1 (KIS-T 68.0\%, KIS-C 94.5\%, AVS 78.4\%, KIS-V 88.0\%) and 0.729 MRR; multi-turn CQR cuts ambiguity to 0.22; fail-closed verification eliminates hallucination (0.0\%); and parallelism reduces p95 latency to 1.85s.
\section{Conclusion}
\label{sec:conclusion}
AEGIS introduces an evidence-carrying, live-first multimodal retrieval architecture for VBS 2027. By coupling 4B WeMM representations with Qdrant HNSW indexing, multi-turn clarification boosting, fail-closed grounded VQA, and interactive timeline exploration, AEGIS delivers sub-second responsiveness with verified accuracy. Future work targets live DRES rehearsal for VBS 2027 at MMM 2027 in Siem Reap, Cambodia, expanded domain collections, and compiler-assisted inference optimization.
