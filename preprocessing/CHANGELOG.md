...
### Fixed - shot boundary layout detection, second pass
- **A single gap no longer discards a whole master shot reference.** Column-pair detection required *every* consecutive row to meet. A shot reference is a partition of the video in principle, but a re-packaging that dropped a few very short shots leaves holes, and one hole anywhere rejected the file outright - losing the official shot identifiers the competition is scored on and falling back to local detection. Most rows must now meet rather than all.
- **A middle-frame or middle-timestamp column can no longer pose as an end column.** It sits inside its own row's segment, so it pairs with the start column exactly as an end column does, and contiguity cannot always separate the two: on a file whose shots run two whole seconds each, the middle timestamp falls exactly one unit before the next start, which is indistinguishable from the inclusive frame-end convention. Whole-number timestamps also make the timestamp columns look integral, which granted them the frame convention's one-unit slack. Extent now settles it on both sides - among pairs sharing a start column the real end is the outer one, among pairs sharing an end column the real start is the earlier one, and a middle column loses both comparisons by construction. Relaxing contiguity is what exposed this; it was latent beforehand for any file with whole-second shot times.

### Changed
- `extract_candidate_frames` reads the frame index back from the decoder instead of counting up from the requested seek position, and skips any frame that falls before the scene. Seeking by frame number is exact on the FFmpeg backend measured here for both intra- and inter-frame codecs, so this is hardening against backends that undershoot rather than a fix for observed drift; a frame from before the scene would otherwise be indexed under two shot ids and give two points the same native frame index.

### Fixed - keyframe extraction reached almost nothing it was meant to
- **Master shot boundary files are now parsed by detecting their column layout instead of assuming one.** `load_shots` took the first two numeric fields on a row as `(start_sec, end_sec)`. The public V3C msb distribution puts both a timestamp pair and a frame number pair on each row, so on a `starttime startframe endtime endframe ...` layout that read `(starttime, startframe)`: the first shot failed the `end > start` check and was dropped, and every later shot became a segment tens of seconds long. Nothing raised - the corpus indexed against nonsense boundaries. `preprocessing/msb_format_detector.py` now identifies the boundary columns from the file's own structure (a column pair is a segmentation only if it increases on every row and is contiguous between rows), and a file carrying only one all-integer pair is reported as ambiguous and resolved from the frame rate or `V3C_MSB_UNITS` rather than guessed at. The previous test fixture used a three-column layout that does not occur in the real distribution.
- **Official keyframes now carry a native frame index.** `load_keyframe_candidate` left `frame_idx` as `None`, which made those points invisible to `temporal_coherence_boost` and `temporal_chain_match` (both skip candidates without one) and made TRAKE emit `"frame_ids": [None, ...]` into the submission. It is now taken from the msb file's own frame columns, or derived from the video frame rate when the file has none.
- **Keyframe filename matching no longer collides on shared prefixes.** `attach_keyframes` fell back to a plain substring test, and `shot00001_1` is contained in `shot00001_10` through `shot00001_19`, so the single-digit shots of any video with ten or more silently lost their keyframe. Matching is now exact, then delimiter-bounded.
- **Named-vector collections accept points that have no secondary vector.** With `SECONDARY_EMBEDDER_ENABLED`, `index_visual_point` sent a bare vector list whenever `secondary_vector` was `None`, which a named-vector collection rejects - failing the whole upsert, not just that one named vector.

### Changed - index budget separated from VLM budget
The keyframe selection stack was inherited from video-LLM papers (AKS, arXiv:2502.21271; SlowFast-LLaVA, arXiv:2407.15841) whose constraint is an LLM context window. Retrieval's constraint is different: a frame that was never indexed can never be retrieved however good the ranking is, while disk and ANN latency are comparatively cheap. The budgets are now separate.

- **Official keyframes anchor the candidate pool instead of replacing it.** When an official keyframe existed, it became the *only* candidate for that shot - after which the DAKE pre-filter was a no-op, `np.var` over one embedding was exactly `0.0` so the adaptive budget was always 1, and farthest-point sampling had nothing to sample. The entire selection stack was unreachable on the corpus it mattered for, and the index was capped at one frame per master shot. The official keyframe is now inserted into the decoded candidate list at its chronological position and forced through both selection passes, so its identifier is still guaranteed to be indexed.
- **Coverage-based index selection** (`select_by_coverage`) decides what gets indexed. Frames are kept until every candidate lies within `KEYFRAME_COVERAGE_TAU` cosine distance of a kept one - greedy k-center, whose 2-approximation guarantee (Gonzalez 1985, doi:10.1016/0304-3975(85)90224-5) is the construction Core-Set active learning uses (Sener & Savarese, ICLR 2018, arXiv:1708.00489). `KEYFRAME_VARIANCE_LOW/MID` no longer govern the index; they still size the VLM tier, where their known weaknesses (a scale tied to one CLIP checkpoint's un-normalized output, and a discontinuous step from 2 to 5 frames at the midpoint) are bounded by `KEYFRAME_VLM_MAX_BUDGET` instead of capping the index. Deriving that budget from the Qwen embeddings already computed, and dropping the separate CLIP variance pass entirely, is the obvious follow-up.
- **Facility location replaces farthest-point sampling** for choosing which frames get described. Dispersion maximisation systematically rewards whatever sits furthest from everything else - dissolves, motion blur, near-black frames - which is what `KEYFRAME_SHARPNESS_WEIGHT` was compensating for. Coverage maximisation picks representatives instead.
- **Each scene is decoded once.** Candidates were decoded at 1fps and the Fast pathway then decoded the same scene again at 8fps and discarded everything it had not motion-sampled. Both now draw from a single decode, so a wider candidate pool costs embedding time rather than video I/O.
- **DAKE only runs once there is enough to thin** (`KEYFRAME_DAKE_MIN_CANDIDATES`). On a ~3.3s master shot it discarded a third of an already tiny candidate list for negligible savings.

### Added
- **Sub-shot splitting** (`SUBSHOT_SPLIT_ENABLED`): result diversification keeps one hit per (video, scene), so an uncut sequence covering several distinct moments could only ever surface one of them. Long shots are split before indexing, with exactly one part claiming the shot's official keyframe.
- **Region-level index points** (`REGION_INDEXING_ENABLED`): the SAM3 crops each described keyframe already produces for detection/OCR gating are embedded and indexed under `modality="region"`. A pooled frame embedding averages a small object away; the crop does not. The separate modality is deliberate - crops share their parent's frame index, so admitting them as ordinary visual points would let a frame with many regions boost itself in temporal coherence, let a crop evict its own parent from the diversified grid, and duplicate frame indices in TRAKE's timeline.
- **`INDEX_SCHEMA_VERSION`** participates in frame point IDs, and `main.py` refuses to start when the collection still holds points from an older schema without `QDRANT_REBUILD_VIDEO_ON_START`. Changing which frames are indexed changes their IDs, so a re-run would otherwise add a second generation of points alongside the first and quietly corrupt any recall measured from it.
