...
- **Coverage-based index selection** (`select_by_coverage`) replaces the variance thresholds. Frames are kept until every candidate lies within `KEYFRAME_COVERAGE_TAU` cosine distance of a kept one - greedy k-center, whose 2-approximation is the construction Core-Set active learning uses (Sener & Savarese, ICLR 2018, arXiv:1708.00489). The old `KEYFRAME_VARIANCE_LOW/MID` thresholds were tied to one CLIP checkpoint's un-normalized output scale and stepped discontinuously from 2 to 5 frames at the midpoint.
